...
registry/<slug>/<slug>.tsx  SOURCE OF TRUTH — what consumers receive
lib/
...
The slug is the contract. It is the folder name, the file name, the export path, the docs
URL, and the `shadcn add` argument. Pick it once, use it everywhere. Kebab-case.

1. **Source** — `registry/<slug>/<slug>.tsx`
2. **Re-export** — `components/ui/<slug>.tsx` containing only
...
5. **Registry item** — add an item to `registry.json` with
   `registryDependencies: ["@b6-ui/tokens"]` and a file `target` of
